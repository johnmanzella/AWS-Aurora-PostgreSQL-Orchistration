   ...
   then
      ...
      then 
         ...
         then
            ...
            CMD="${ZBASEDIR}/bin/mysqladmin ${ZCONN} stop-slave > ${TMPLOG}.out"
            #eval ${CMD} </dev/null >/dev/null 2>&1 & disown "$!"
            eval ${CMD} 1>>${DEBUG_LOG} 2>&1
            ZTMP=`cat ${TMPLOG}.out`
            log "Shutdown Slave Results: ${ZTMP}"
         fi
...
startDatabase() {
   ...
   then 
      ...
      CMD="${ZCMD} ${ZOPTIONS} --defaults-file=${ZMYCNF} --basedir=${ZBASEDIR} --datadir=${ZDATADIR} --pid-file=${ZPIDFILE} --port=${ZPORT} --socket=${ZSOCKET} --tmpdir=${ZTMPDIR}"
      ...
      then
         ...
         # Start Slave ...
         #
         TMPLOG="${DLPX_TOOLKIT}/start-slave_$$"
      ...
      log "Warning: Start Database aborted since status is not ACTIVE ..."
...
buildConnectionString () {
   ...
   then
      ZSTRING="-u${ZCONN} -p --protocol=TCP --port=${ZPORT} --host=localhost"
   fi
   if [[ ! "${ZCONN}" = *"-u"* ]] && [[ "${ZPASS}" != "" ]]
   then
      ZSTRING="-u${ZCONN} -p${ZPASS} --protocol=TCP --port=${ZPORT} --host=localhost"
   fi
